...
# ── Ontology download ──
def init_ontology(obo_url="https://purl.obolibrary.org/obo/hp.obo", obo_path="hp.obo", refresh_days=14):
...
def vectorize_dataframe(df, meta_out, vec_out):
    print("Loading SBERT model: pritamdeka/SapBERT-mnli-snli-scinli-scitail-mednli-stsb")
...
if __name__ == '__main__':
    ...
    print(f"Built DataFrame with {len(df)} rows -> hpo_terms_full.csv")
    vectorize_dataframe(df, meta_out='hpo_meta.json', vec_out='hpo_embedded.npz')
